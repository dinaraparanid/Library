...
namespace booksys
{
    ...
    public:
        TheBook(QString&& title, QString&& author, QSet<QString>&& genres)                noexcept :
            title_(title), author_(author), genre_(genres) {}
        TheBook(const QString& title, const QString& author, const QSet<QString>& genres) noexcept :
            title_(title), author_(author), genre_(genres) {}
        ...
        [[nodiscard]] QString getTitle()        const noexcept { return title_; }
        [[nodiscard]] QString getAuthor()       const noexcept { return author_; }
        [[nodiscard]] QSet<QString> getGenres() const noexcept { return genre_; }

        void changeTitle(QString&& title)        noexcept { title_ = title; }
        void changeTitle(const QString& title)   noexcept { title_ = title; }
        void changeAuthor(QString&& author)      noexcept { author_ = author; }
        void changeAuthor(const QString& author) noexcept { author_ = author; }
        void addGenre(QString&& genre)           noexcept { genre_.insert(genre); }
        void addGenre(const QString& genre)      noexcept { genre_.insert(genre); }
        void removeGenre(QString&& genre)        noexcept { genre_.remove(genre); }
        void removeGenre(const QString& genre)   noexcept { genre_.remove(genre); }
